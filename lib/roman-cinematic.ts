// Ordered line geometry for the cinematic "self-drawing" Roman hero.
//
// Everything is generated as polylines ("pieces") in a shared design space and
// returned in the exact order the glowing line should trace them: the ornate
// scutum first, then a gladius with crossed spears beside it. Each piece also
// carries a small z so the composition reads with depth/parallax. No art files.
...
function swordGroup(scale: number, ox: number, oy: number): Piece[] {
  const S = (p: Pt): Pt => [p[0] * scale + ox, p[1] * scale + oy]
  const map = (pts: Pt[]) => pts.map(S)
  const pieces: Piece[] = []

  // crossed spears behind the sword (drawn first, slightly deeper)
  const spear = (ang: number): Pt[] => {
    const raw: Pt[] = [[0, -2.6], [0, 2.4]]
    const head: Pt[] = [
      [0, 2.4],
      [-0.22, 1.95],
      [0, 2.9],
    ...
    return [...raw.map((p) => rotate(p, ang)), ...head.map((p) => rotate(p, ang))]
  }
  pieces.push({ pts: map(spear(D(24))), z: -0.35 })
  pieces.push({ pts: map(spear(-D(24))), z: -0.35 })

  // gladius (short sword) in front
  pieces.push({ pts: map([[0, 2.25], [-0.24, -0.4], [0.24, -0.4], [0, 2.25]]), z: 0.15 }) // blade
  pieces.push({ pts: map([[0, 2.0], [0, -0.4]]), z: 0.16 }) // fuller
  pieces.push({
    pts: map([[-0.85, -0.42], [0.85, -0.42], [0.85, -0.72], [-0.85, -0.72], [-0.85, -0.42]]),
    z: 0.16,
  }) // crossguard
  pieces.push({ pts: map([[-0.13, -0.72], [-0.13, -1.66]]), z: 0.16 }) // grip
  pieces.push({ pts: map([[0.13, -0.72], [0.13, -1.66]]), z: 0.16 })
  pieces.push({ pts: map([[-0.13, -1.12], [0.13, -1.12]]), z: 0.16 })
  pieces.push({ pts: map([[-0.13, -1.4], [0.13, -1.4]]), z: 0.16 })
  pieces.push({ pts: map(circle(0, -1.9, 0.27, 36)), z: 0.16 }) // pommel
  return pieces
}

// ---- the full ordered scene -----------------------------------------------

function rawScene(): Piece[] {
  const p: Piece[] = []

  // 1) shield barrel outline as one continuous loop
  const outline: Pt[] = [
    [-1.55, 2.15],
    [1.55, 2.15],
  ...
  // 8) filigree flourishes in the upper corners
  p.push({ pts: spiral(-1.05, 1.5, 0.3, 1.4, 1, 34), z: 0.05 })
  p.push({ pts: spiral(1.05, 1.5, 0.3, 1.4, -1, 34), z: 0.05 })

  // 9) gladius + crossed spears beside the shield
  for (const piece of swordGroup(0.62, 3.35, -0.1)) p.push(piece)

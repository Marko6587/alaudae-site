...
export default function RomanLineCinematic({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // First pass: resample and measure total length.
    ...
    const resize = () => {
      ...
      camera.aspect = w / h
      // frame the composition regardless of aspect
    ...
    const render = () => {
      ...
      if (!reduced) {
        const c = now % CYCLE
        if (c < DRAW) {
          progress = smoother(c / DRAW)
          alpha = Math.min(1, c / 0.6) // fade-in at the start of each cycle
        } else if (c < DRAW + HOLD) {
          progress = 1
          alpha = 1
        } else {
          progress = 1
          alpha = 1 - (c - DRAW - HOLD) / FADE
        }
      }
      const revealed = progress * totalLen

      // reveal each line + carry head position
      let headSet = false
      ...
      // living haze / streaks (kept subtle so the line work stays crisp)
    ...
    return (
      <div className={className} aria-hidden="true">
        <svg viewBox={viewBox} className="h-full w-full" preserveAspectRatio="xMidYMid meet">
          {polylines.map((pts, i) => (
            <polyline key={i} points={pts} fill="none" stroke="#c8862f" strokeWidth={0.03} strokeLinecap="round" strokeLinejoin="round" opacity={0.9} />
          ))}
        </svg>
      </div>
    )
